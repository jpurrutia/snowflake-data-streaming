...
def s3_date_key():
    ...
    unix_path = now.timestamp()
    path = '{0}-{1}-{2}_{3}-{4}-{5}'.format(
        now.year,
        now.month,
        now.day,
        now.hour,
...
def lambda_handler(event, context):
	...
	try:
		connection = psycopg2.connect(user=rds_config.db_username,
									  password=rds_config.db_password,
									  host=rds_config.db_endpoint,
									  port=rds_config.db_port,
									  database=rds_config.db_name)
		
		cursor = connection.cursor() # conneciton.cursor(cls,idx) "cls" when specified creates and "iteration" cursor, which will iterate over all obj of class cls
	   								 # id tused to iterate over all objects of the class cls; if not specified an autoid/list index will be used - all connected to postgres
...
##############################################################################################################
		try:
			# Capture the entire event in a variable
			data = ebody
			# Set the overall bucket
			bucket = "iw-servercareupload"
			# Capture the timestamp of the triggering event
			key_prefix, file_time = s3_date_key()
			# Derive the file name
			file_name = '{0}_{1}.json'.format(data_cc, file_time)
			# Derive the S3 object key
			key = 'raw-events/{0}/{1}/{2}/{3}/{4}'.format(org_name, proj_name, proj_id, key_prefix, file_name)
			# Put the event data to a file object in S3
			s3.put_object(Body=json.dumps(ebody), Bucket=bucket, Key=key)
			# Log the results
			logger.info('Loaded to S3: {0}/{1}'.format(bucket, key))
...
##############################################################################################################
		...
		# end of linux
			...
			for c in ebody["counters"]:
				ts = datetime.datetime.fromtimestamp(int(c["Timestamp"][6:19])/1000).strftime('%Y-%m-%d %H:%M:%S')
				pathArr = c["Path"].split('\\')
				hn = pathArr[2]
				pa3 = pathArr[3].split('(')
				m1 = pa3[0]
				m2 = pathArr[4]
				...
				# if value is >50MB and process memory
				if not ( m1 == "process" and (val == 0 or (m2 == "private bytes" and val < 50000000))):

					try:
						insq += "(%s, '%s', '%s', '%s', '%s', '%s', '%s', %s)," % (proj_id, hn, ts, m1, m2, m3, m4, str(val))
			...
			try:
				print (eb["osver"])
				insq += "(%s, '%s', '%s', '%s', '%s', '%s', '%s', %s)," % (proj_id, hn, ts, 'os', 'version', ebody["osver"], '', '1')
			except:
				pass
			
			try:
				insq += "(%s, '%s', '%s', '%s', '%s', '%s', '%s', %s)," % (proj_id, hn, ts, 'memory', 'Installed RAM (GB)', '', '', str(ebody["ramgb"]))
			except:
				pass
			
			try:
				insq += "(%s, '%s', '%s', '%s', '%s', '%s', '%s', %s)," % (proj_id, hn, ts, 'processor', 'CPU Cores', '', '', str(ebody["proc_cores"]))
			except:
				pass
			
			try:
				insq += "(%s, '%s', '%s', '%s', '%s', '%s', '%s', %s)," % (proj_id, hn, ts, 'processor', 'Logical Processors', '', '', str(ebody["proc_lp"]))
			except:
				pass
			
			try:
				insq += "(%s, '%s', '%s', '%s', '%s', '%s', '%s', %s)," % (proj_id, hn, ts, 'system', 'Uptime (Days)', '', '', str(ebody["updays"]))
			except:
				pass
		...
		try:
			...
			try:
				...
				hr_ts = datetime.datetime.strptime(ts,"%Y-%m-%d %H:%M:%S").replace(minute=0, second=0, microsecond=0)
				# Add to hours table
				sumq = "insert into proj_host_hours (project_id, hostname, hour_ts) values (%s, '%s', '%s') on conflict on constraint proj_host_hours_pk do nothing" % (proj_id, hn, hr_ts)
				cursor = connection.cursor()
				cursor.execute(sumq)
				connection.commit()
